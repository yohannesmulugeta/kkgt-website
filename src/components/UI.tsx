...
export function PageHero({ eyebrow, title, accent, copy, image }: { eyebrow: string; title: string; accent?: string; copy: string; image: string }) {
  return (
    <section className="page-hero">
      <div className="page-hero__image" style={{ backgroundImage: `linear-gradient(90deg, rgba(4,40,24,.9), rgba(4,40,24,.25)), url(${image})` }} />
  ...
  return (
    <section className="inquiry-band">
      <div className="container inquiry-band__grid">
        ...
        <Link to="/contact" className="round-action" aria-label="Contact KKGT"><ArrowRight size={28} /></Link>
      </div>
    </section>
  );
}
...
export function ProductCard({ product }: { product: AgroProduct }) {
  ...
  return (
    <Link to={`/agrochemicals/product/${product.slug}`} className="product-card">
      ...
      <div className="product-card__visual"><span>{product.name.slice(0, 2).toUpperCase()}</span></div>
      <div className="product-card__body">
        ...
        <div className="product-card__status"><ShieldCheck size={15} /> Technical claims shown only when verified</div>
      </div>
    </Link>
  );
}